...
public class Debit implements Payment {
	public boolean pay(int amount, Account sender, Account receivere) {
		System.out.println("Payment by debit Card Started");
		try {
			Thread.sleep(3000);
	...
	public boolean validateDetails(PaymentDetails detail) {
		...
		try {
			if (!(detail instanceof DebitCardDetails)) {
				System.out.println("Invalid details type");
				return false;
			}

			DebitCardDetails debitCardDetail = (DebitCardDetails) detail;

           System.out.println();
			if (!debitCardDetail.getCvv().matches("\\d{3}")) {
				System.out.println("INVAID CVV");
				return false;
			}

			YearMonth expiryDate = YearMonth.parse(debitCardDetail.getExpiry(), DateTimeFormatter.ofPattern("MM/yy"));
			if (expiryDate.isBefore(YearMonth.now())) {
				System.out.println("INVALID Expiry");
				return false;
			}

			if (!debitCardDetail.getCardnNumber().matches("\\d{16}")) {
